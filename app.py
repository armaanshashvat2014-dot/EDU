...
def is_image_mime(m: str) -> bool:
    m = (m or "").lower()
    return m.startswith("image/") or m in {"image/svg+xml"}
...
if user_role == "teacher":
            ...
            with st.spinner(f"Reading {assign_grade} {assign_subject} curriculum and writing paper..."):
                try:
                    ...
                    gen_prompt = (
                        ...
                        """QUESTION PAPERS (CRITICAL FORMATTING)
    ...
    - ENGLISH PAPERS: There are 2 papers, formal and informal. Make both if not specified. There must be at least 15 questions per paper, and a paper is 40M for grade 7 and below, 50m for grade 8. Each question must have at least 2 or more bits. The grammar must be related to the text. Each reading comprehension text must be 700+ words. Also add things like poem comprehension, and more if the syllabus includes it (200 words max). It should be detailed and big, kinda hard. Create your own text for these. Use both creative writings if the exact exam syllabus is given. BOTH TEXTS IS MANDARORY. for grammar each q MUST HAVE 3 of the SAME GRAMMAR BITS, AND ALL GRAMMAR MUST BE PRESENT. Give 2 writing tasks, both must be completed by the student. (If a photo if syllabus is given choose from that or choose two from the book)
...
if render_chat_interface:
    ...
    - PDF TRIGGER: If, and ONLY IF, you generated a full formal question paper, append [PDF_READY] at the very end.
    - SYLLABUS STRICTNESS: When generating a question paper, every single question MUST be sourced directly from the attached textbook context. If the user asks for "Chapter n", find Chapter n in the text and only use topics from that chapter.
    - ENGLISH PAPERS: There are 2 papers, formal and informal. Make both if not specified. There must be at least 15 questions per paper, and a paper is 40M for grade 7 and below, 50m for grade 8. Each question must have at least 2 or more bits. The grammar must be related to the text. Each reading comprehension text must be 750+ words. Also add things like poem comprehension, and more if the syllabus includes it (200 words max). It should be detailed and big, kinda hard.  Use both creative writings if the exact exam syllabus is given. BOTH TEXTS IS MANDARORY Give 2 writing tasks, both must be completed by the student. (If a photo if syllabus is given choose from that or choose two from the book)
