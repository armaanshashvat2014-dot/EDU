...
def select_relevant_books(query, file_dict):
    ...
    s7, s8, s9 = "stage 7" in qn, "stage 8" in qn, "stage 9" in qn
    im, isc, ien = any(k in qn for k in["math", "algebra", "number"]), any(k in qn for k in["science", "biology"]), any(k in qn for k in["english", "poem"])
    if not (s7 or s8 or s9): s8 = True
    if not (im or isc or ien): im = isc = ien = True
    sel =[]
    def add(k, act):
        if act: sel.extend([b for b in file_dict.get(k,[]) if (s7 and "cie_7" in b.display_name.lower()) or (s8 and "cie_8" in b.display_name.lower()) or (s9 and "cie_9" in b.display_name.lower())])
